...
public class BinarySearch {
    ...
    private int binSearch(ArrayList<Integer> toSearch, int whichNum, int index, int numComparisons){
        ...
        if(toSearch.size() ==1){ //if the array is just one element
            ...
            assert (toSearch.get(0) == whichNum && index > 0);
            return index;
        }
        else{
            ...
            if(value > whichNum){ //if the midpoint is greater than the target, the target is to the left
                ...
                index+= toSearch.size() - right.size();
    ...
    /**
     * autoTest: a method to test the binary search method n many times
     * @param howManyTests the number of tests
     */
    public void autoTest(int howManyTests){
        //create an array of the numbers 0-(n-1)
        ...
        int pass = 0;
        int fail = 0;
//then call binary search with each element being the target
        for(int i = 0; i < howManyTests; i++){
            System.out.println("Target is: "+ i);
            BinarySearch runner = new BinarySearch(toSend);
            int result = runner.binSearch(toSend,i, 0, 0 );
            if (result == i){
               pass++;
               continue;
            }
        ...
        System.out.println(pass + " tests passed and " + fail + " tests failed");
